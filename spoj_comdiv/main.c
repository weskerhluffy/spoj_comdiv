...
// XXX: https://www.spoj.com/problems/COMDIV/
...
#if 1 /* comdiv */
#define COMDIV_MAX_PRIMO ((natural)1E6)
typedef struct cuenta_divisores{
    entero_largo_sin_signo cuentas_divisores[COMDIV_MAX_PRIMO+1];
}cuenta_divisores;

void comdiv_primo_encontrado(natural primo,
                           natural idx_primo,
                           void *cb_ctx){
    
    cuenta_divisores *sd=cb_ctx;
    entero_largo_sin_signo nueva_contribucion_primo=2;
    
    sd->cuentas_divisores[primo]=nueva_contribucion_primo;
}

void comdiv_divisible_encontrado (natural primo,
                                natural idx_primo,
                                natural compuesto,
                                void *cb_ctx){
    cuenta_divisores *sd=cb_ctx;
    entero_largo_sin_signo *cuentas_divisores=sd->cuentas_divisores;
    ...
    suma_divisores_compuesto=cuentas_divisores[compuesto];
    
    complemento_contribucion_primo=suma_divisores_compuesto/contribucion_primo;
    
    nueva_contribucion_primo=contribucion_primo+1;
    
    suma_divisores_nuevo_compuesto=complemento_contribucion_primo*nueva_contribucion_primo;
    
    cuentas_divisores[nuevo_compuesto]=suma_divisores_nuevo_compuesto;
}

void comdiv_no_divisible_encontrado (natural primo,
                                   natural idx_primo,
                                   natural compuesto,
                                   void *cb_ctx){
    cuenta_divisores *sd=cb_ctx;
    ...
    suma_divisores_nuevo_compuesto=cuentas_divisores[compuesto]*nueva_contribucion_primo;
    
    cuentas_divisores[nuevo_compuesto]=suma_divisores_nuevo_compuesto;
}


COMUN_FUNC_STATICA void comdiv_main(){
    natural t=0;
    primos_datos *pd=NULL;
    cuenta_divisores *sd=NULL;
    
    sd=calloc(1, sizeof(cuenta_divisores));
    assert_timeout(sd);
    
    sd->cuentas_divisores[1]=1;
    
    pd=calloc(1, sizeof(primos_datos));
    assert_timeout(pd);
    
    primos_criba_criba(COMDIV_MAX_PRIMO, comdiv_primo_encontrado, NULL, comdiv_divisible_encontrado, comdiv_no_divisible_encontrado, sd, pd);
    ...
    while(t--){
        ...
        scanf("%u %u",&a,&b);
        g=comun_mcd(a, b);
        natural r=sd->cuentas_divisores[g];
        printf("%u\n",r);
    }
}
